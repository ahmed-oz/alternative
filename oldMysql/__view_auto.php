            ...
            <div class="container">
				...
				// permette ad Alternativa di visualizzare tutte le auto anche bloccate, vendute, ecc..
					if ($_SESSION['id_livello_sessione'] >= 3) {
						$query = @mysql_query("SELECT auto.*, concessionari.nome_concessionaria , auto.id_status as autoidstatus FROM auto, concessionari WHERE concessionari.id_concessionaria = auto.id_concessionario AND id = $id_auto");
					} else {
						$query = @mysql_query("SELECT auto.*, concessionari.nome_concessionaria, auto.id_status as autoidstatus FROM auto, concessionari WHERE concessionari.id_concessionaria = auto.id_concessionario AND id = $id_auto AND auto.id_status >= 1");
					}
                    ...
                    <div class="mws-panel-body no-padding">
                        <table class="mws-table">
                            <tbody>
                				...
                				<tr>
                                    <td>alimentazione</td>
									<td><?php $res = mysql_query("SELECT nome_alimentazione FROM alimentazione WHERE id='$alimentazione';");
									echo (@mysql_result($res,0,0))?:'---'; ?></td>
                                </tr>
								<tr>
				...
				<div class="mws-panel grid_4 mws-collapsible">
					...
					if ($num == 0)
					{
						...
						while ($i < $num)
						{
							$id_doc=mysql_result($risultati,$i,"id_documento");
							$nome_documento=mysql_result($risultati,$i,"nome_documento");
							$descrizione_documento= ( mysql_result($risultati,$i,"descrizione_documento") == NULL) ? "Visualizza documento" : mysql_result($risultati,$i,"descrizione_documento");
							?>					
							<p>
