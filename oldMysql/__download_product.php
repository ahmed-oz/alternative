...
$result = mysql_query("SELECT marca FROM auto WHERE id=$id_auto");
if(mysql_num_rows($result)) $name = mysql_result($result,0,0);
?>
            	...
            	<div class="mws-panel grid_8">
                    ...
                    <div class="mws-panel-body no-padding">
                        ...
                        <table id="download-product-table" class="mws-datatable-fn mws-table">
							...
							$i = 0;
							while($i < $num) {
								$id_download = mysql_result($result,$i,"id");
								$link = mysql_result($result,$i,"link");
								$id_category = mysql_result($result,$i,"id_category");
								$permission = mysql_result($result,$i,"type_level");
								$status = mysql_result($result,$i,"status");
								
								$cat_res = @mysql_query("SELECT name_en FROM download_category WHERE id_download_category = $id_category;"); // ********** name_en **********
								$category = @mysql_result($cat_res,0,"name_en"); // **********
							?>
                                <tr>
                                    <td><?= $id_download; ?></td>
									<td><?= $link; ?></td>
