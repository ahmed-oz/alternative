...
$query = @mysql_query("SELECT auto.*, concessionari.nome_concessionaria, auto.id_status as autoidstatus FROM auto, concessionari WHERE concessionari.id_concessionaria = auto.id_concessionario AND id = {$id}");
        ...
        <tbody>
			...
			<tr>
				<td colspan="2">
					...
					$risultati_equipaggiamento=mysql_query($query_equipaggiamento);
					$num=mysql_numrows($risultati_equipaggiamento);
					if ($num > 0) {
						$i=0;
						while ($i < $num) {
							$accessorio=mysql_result($risultati_equipaggiamento,$i,"accessorio");
							echo $accessorio . "; ";
							$i++;
						}
					}
